import torch
from ._vanilla_utils import init_plan, compute_local_cost, log_sinkhorn, exp_sinkhorn


def log_ugw_sinkhorn(
    a,
    dx,
    b,
    dy,
...
def exp_ugw_sinkhorn(
    ...
    for i in range(nits_plan):
        ...
        mp = pi.sum()
        ecost = (
            -compute_local_cost(pi, a, dx, b, dy, eps, rho, rho2) / (mp * eps)
        ).exp()
